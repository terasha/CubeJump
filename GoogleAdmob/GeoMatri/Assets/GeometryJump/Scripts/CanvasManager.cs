...
namespace NNest.GeometryJump
{
	public class CanvasManager : MonoBehaviorHelper 
	{
		...
		[SerializeField] private Button buttonAchive;
		...
		void RemoveListener()
		{
            ...
            buttonAchive.interactable = false;
		...
		public void OnClickedButton(GameObject b)
		{
            ...
            else if (b.name.Contains("ButtonRanking"))
                OnClickedRanking();
            else if (b.name.Contains("ButtonGet300Diamonds"))
                OnClickedGetFreeDiamonds();
            else if (b.name.Contains("ButtonRestart"))
                OnClickedRestart();
            else if (b.name.Contains("ButtonMask"))
                OnClickedButtonMask();
            else if (b.name.Contains("ButtonHome"))
                OnClickHome();
            else if (b.name.Contains("ButtonRanking"))
                OnClickRanking();
            else if (b.name.Contains("ButtonAchive"))
                OnClickAchive();
        }
        ...
        void OnClickAchive()
        {
            LoginManager.instnace.ShowAchiveBoard();
        }
		...
		void OnClickedRanking()
		{
			RemoveListener();
            LoginManager.instnace.ShowLeaderBoard(GPGSIds.leaderboard_ranking);
        }
...
#endif
            UnityAdsManager.instnace.ShowRewardedAd((result) => {
                switch (result)
                {
                    case UnityEngine.Advertisements.ShowResult.Finished:
                        ...
                        ButtonLogic();
                        AddButtonListener();
                        ActivateButton(buttonGetFreeDiamonds, false);
                        break;
